...
class Creator:
    ...
    Methods
    -------
    - create_widget_layout(parent_window, layout_config)
    ...
    def __create_widget(
        ...
        класс и метод создания. Если конфиге оказался контейнер, вызывает метод
        создания контейнера вместо этого.
        На данный момент поддерживается 6 типов виджетов:
        Лейбл, Поле для ввода, Поле для выбора, Чекбокс, Кнпока, Картинка.
    ...
    def __add_border_frame(self, layout: QLayout) -> QFrame:
        ...
        -------
        - frame: QFrame
            Фрейм с обернутым в него контейнером.
        """
